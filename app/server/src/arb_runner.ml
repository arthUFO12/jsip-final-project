...
(* ---------- The in-app strategy runner ---------- *)

(* One runner per server, ticking [scan] on an interval. [Paper] only
   records (wallet paper entries + observations, as every scan does).
   [Armed] additionally auto-fires the KALSHI LEG ONLY of each fresh
   tradable edge — through the same executor, rails, and audit trail as
   the assisted hedge — until the run's budget of notional is spent. The
   position is deliberately one-sided (Polymarket never auto-trades); the
   audit rows and the UI both say so. *)
module Live_run_state = struct
  type t =
    { mutable running : bool
    ; mutable stop_requested : bool
    ; mutable mode : Protocol.Live_run.Mode.t
    ; mutable interval_s : int
    ; mutable ticks : int
    ; mutable last_tick : Time_ns.t option
    ; mutable last_summary : string
    ; mutable spent_dollars : float
    ; mutable fired_pairs : String.Set.t
      (** fired once per pair per run, however long the edge persists *)
    }
end

let live_run : Live_run_state.t =
  { running = false
  ...
  ; ticks = 0
  ; last_tick = None
  ; last_summary = "never run"
  ; spent_dollars = 0.
  ; fired_pairs = String.Set.empty
  }
;;

let live_run_status () =
  Deferred.Or_error.return
    { Protocol.Live_run.Status.running = live_run.running
    ; mode = live_run.mode
    ; interval_s = live_run.interval_s
    ; ticks = live_run.ticks
    ; last_tick_s =
        Option.map live_run.last_tick ~f:(fun time ->
          Time_ns.Span.to_sec (Time_ns.to_span_since_epoch time))
    ; last_summary = live_run.last_summary
    ; spent_dollars = live_run.spent_dollars
    }
;;

(* Fire the Kalshi leg of one edge, inside every rail. Returns what
   happened as a string for the tick summary; only [`Fired] spends. *)
let auto_fire_edge (edge : Protocol.Edge_card.t) =
  let open Deferred.Or_error.Let_syntax in
  match !live_state with
  | None -> return (`Skipped "key locked")
  | Some (executor, credentials) ->
    let budget_room =
      match live_run.mode with
      | Armed { budget_dollars } -> budget_dollars -. live_run.spent_dollars
      | Paper -> 0.
    in
    let%bind stub = hedge_stub_of_pair_key edge.pair_key in
    let contract : Contract_type.t =
      match String.equal edge.yes.venue "Kalshi" with
      | true -> Yes
      | false -> No
    in
    let%bind ask, depth = hedge_quote stub ~contract in
    let stake = Size.to_int !execution_config.stake_per_opportunity in
    let size = Int.min stake depth in
    (match size <= 0 with
     | true -> return (`Skipped "no depth")
     | false ->
       let order =
         { Execution.Order.market = stub
         ; contract
       ...
       in
       let dollars = Rails.order_dollars order in
       (match Float.O.(dollars > budget_room) with
        | true -> return (`Skipped "run budget exhausted")
        | false ->
          let now_ms =
            Time_ns.to_int_ns_since_epoch (Time_ns.now ()) / 1_000_000
          in
          let client_order_id = [%string "auto-%{now_ms#Int}"] in
          (match%bind.Deferred
             Rails.live_refusal ~execution:!execution_config order
           with
           | Some reason ->
             let%bind () =
               Deferred.ok
                 (Rails.log_live_order
                    order
                    ~client_order_id
                    ~outcome:[%string "auto refused: %{reason}"]
                    ~dollars:0.)
             in
                ...
                let%bind () =
                  Deferred.ok
                    (Rails.log_live_order
                       ...
                       ~outcome:
                         [%string
                           "auto error: %{Error.to_string_hum error}"]
                       ~dollars:0.)
                in
                return (`Skipped (Error.to_string_hum error))
              | Ok fill ->
                let filled = Size.to_int fill.filled_size in
                let%bind filled =
                  match filled < size, fill.venue_order_id with
                ...
                let%bind () =
                  Deferred.ok
                    (Rails.log_live_order
                       ...
                       ~outcome:
                         [%string
                           "auto: filled %{filled#Int} - ONE-SIDED Kalshi \
                            leg; the Polymarket leg is yours to take"]
                       ~dollars)
                in
                live_run.spent_dollars <- live_run.spent_dollars +. dollars;
                return (`Fired dollars)))))
;;

let live_run_tick () =
  match%bind.Deferred scan () with
  | Error error ->
    live_run.last_summary
      <- [%string "scan failed: %{Error.to_string_hum error}"];
    Deferred.unit
  | Ok report ->
    let base =
      [%string
        "%{report.pairs#Int} pair(s), %{report.tradable#Int} tradable"]
    in
    (match live_run.mode with
     | Paper ->
       live_run.last_summary <- [%string "paper tick: %{base}"];
       Deferred.unit
     | Armed { budget_dollars = (_ : float) } ->
       let fresh =
         List.filter report.edges ~f:(fun edge ->
           edge.tradable
           && (not edge.acted)
           && not (Set.mem live_run.fired_pairs edge.pair_key))
       in
       let%map.Deferred fired, skipped =
         Deferred.List.fold
           fresh
           ~init:(0, [])
           ~f:(fun (fired, skipped) edge ->
             live_run.fired_pairs
               <- Set.add live_run.fired_pairs edge.pair_key;
             match%map.Deferred auto_fire_edge edge with
             | Ok (`Fired (_ : float)) -> fired + 1, skipped
             | Ok (`Skipped reason) -> fired, reason :: skipped
             | Error error ->
               fired, Error.to_string_hum error :: skipped)
       in
       let skips =
         match skipped with
         | [] -> ""
         | reasons ->
           [%string
             "; skipped: %{String.concat ~sep:\"; \" (List.rev reasons)}"]
       in
       live_run.last_summary
         <- [%string
              "ARMED tick: %{base}; fired %{fired#Int}, \
               $%{sprintf \"%.2f\" live_run.spent_dollars} of budget \
               spent%{skips}"])
;;

let rec live_run_loop () =
  match live_run.stop_requested with
  | true ->
    live_run.running <- false;
    live_run.stop_requested <- false;
    Deferred.unit
  | false ->
    let%bind.Deferred () = live_run_tick () in
    live_run.ticks <- live_run.ticks + 1;
    live_run.last_tick <- Some (Time_ns.now ());
    (match live_run.stop_requested with
     | true ->
       live_run.running <- false;
       live_run.stop_requested <- false;
     ...
     | false ->
       let%bind.Deferred () =
         Clock_ns.after (Time_ns.Span.of_int_sec live_run.interval_s)
       in
       live_run_loop ())
;;

let start_live_run
  ({ interval_s; mode } : Protocol.Live_run.Start_request.t)
  =
  let open Deferred.Or_error.Let_syntax in
  match live_run.running with
  | true ->
    Deferred.Or_error.error_string
      "the runner is already running - stop it first"
  | false ->
    let%bind () =
      match mode with
      | Paper -> return ()
      | Armed { budget_dollars } ->
        (match !live_state with
         | None ->
           Deferred.Or_error.error_string
             "arming requires an unlocked trading key - unlock one on the \
              Wallet page"
         | Some (_ : Execution.Executor.t * Execution.Kalshi_live.Credentials.t)
           ->
           (match Float.O.(budget_dollars > 0.) with
            | true -> return ()
            | false ->
              Deferred.Or_error.error_string
                "the run budget must be positive"))
    in
    live_run.running <- true;
    live_run.stop_requested <- false;
    live_run.mode <- mode;
    live_run.interval_s <- Int.clamp_exn interval_s ~min:10 ~max:3600;
    live_run.ticks <- 0;
    live_run.last_tick <- None;
    live_run.spent_dollars <- 0.;
    live_run.fired_pairs <- String.Set.empty;
    live_run.last_summary <- "starting...";
    don't_wait_for (live_run_loop ());
    live_run_status ()
;;
...
let stop_live_run () =
  (match live_run.running with
   | true ->
     ...
     live_run.last_summary <- "stopping after the current tick..."
   | false -> ());
  live_run_status ()
;;

(* What taking every fresh edge once would have locked in, as the scans
   actually saw them — the forward, real-data counterpart of the
   backtest's cumulative line. *)
let arb_history () =
  let open Deferred.Or_error.Let_syntax in
  let%map sightings = Database.Database_exec.list_arb_observations () in
  Replay.observation_episodes
    ~sightings:
      (List.map
         sightings
         ~f:(fun { Arb_observation.at; pair_key; edge = _; size = _; dollars }
            ->
           ( Time_ns.Span.to_sec (Time_ns.to_span_since_epoch at)
           , pair_key
           , dollars )))
    ~gap_s:900.
;;
