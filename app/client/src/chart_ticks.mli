(** Axis tick placement for {!Chart}: "nice" y values on a 1-2-5 ladder and
    UTC-labeled time ticks for the x axis. Pure — the Bonsai layer draws
    the gridlines and text nodes. *)

open! Core

(** At most [max_count] tick values inside the range, stepped by the
    smallest 1-2-5 x 10^k step that fits, each an exact multiple of the
    step. Empty when the range is degenerate ([hi <= lo]). *)
...
(** Time ticks for a range in epoch seconds: [(time_s, label)] pairs on a
    ...
    Labels are ["Jul 9"] for day-or-longer steps and ["Jul 9 06:00"] below
    a day. Empty when the range is degenerate. *)
val time_ticks
  :  Chart.Range.t
  -> max_count:int
  -> (float * string) list
