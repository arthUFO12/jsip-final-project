(** The rule editor's {!Autocomplete} environment and its click-to-insert
    ...
    The environment holds the bot language's keywords, the builtin
    [$variables], the chosen market tickers, and — like the parser's
    [Var_env] — a [$name] for every [name = ...] definition already in the
    buffer, so a variable becomes completable the moment it is defined.
    Rebuild it on every keystroke; it is cheap. *)
