...
(** Encrypted at-rest storage for the user's Kalshi trading key — the disk
    ...
    the passphrase (PBKDF2-HMAC-SHA256 then AES-256-GCM), and writes one
    0600 file under the user's home directory. Every later session,
    {!unlock} takes the passphrase again and rebuilds in-memory
    {!Kalshi_live.Credentials.t}; the plaintext PEM never touches disk after
    the initial paste, and a wrong passphrase fails the GCM tag check
    loudly. {!forget} deletes the file.
...
(** [save ~path ~key_id ~private_key_pem ~passphrase ~production ()]
    validates the PEM by constructing credentials for the right host,
    refuses passphrases under 8 characters, then encrypts and writes the
    wallet file (0600, directory created if needed). Overwrites a previous
    wallet — connecting a new key replaces the old one. *)
val save
  :  path:string
  -> key_id:string
...
(** [unlock ~path ~passphrase ()] decrypts the stored PEM and rebuilds
    credentials for the host recorded at {!save} time. A wrong passphrase
    (or tampered file) fails the authenticated decryption and is reported as
    exactly that. *)
val unlock
  :  path:string
  -> passphrase:string
